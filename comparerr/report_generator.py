...
class ReportGenerator:
    ...
    def analyze_version(self, ref):
        ...
        # clone repository and reset to git reference
        git.Git(working_dir=workdir).clone(os.path.join(self.location), "./")
    ...
    def _get_messages(self, path):
        ...
        for target in self.targets:
            abspath_targets.append(os.path.join(path, target))
        args = ["-sn",
                "--exit-zero",
                ]
        if self.error_only:
